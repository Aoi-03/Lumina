...
class Ray:
    def __init__(self, pos, dir_vec, black_holes):
        self.x = pos[0]
        self.y = pos[1]
        self.black_holes = black_holes
        
        # Convert to polar coordinates (relative to first black hole)
        ...
        # Store conserved quantities (relative to first black hole)
        self.L = self.r * self.r * self.dphi
        f = 1.0 - black_holes[0].r_s / self.r
    ...
    def step(self, dlambda):
        # Check if inside any event horizon
        for bh in self.black_holes:
            dist = math.sqrt((self.x - bh.position[0])**2 + (self.y - bh.position[1])**2)
            if dist <= bh.r_s:
                return
        
        rk4_step(self, dlambda, self.black_holes)
...
def geodesic_rhs(ray, black_holes):
    r = float(ray.r)
    dr = float(ray.dr)
    dphi = float(ray.dphi)
    E = float(ray.E)
    
    # Use primary black hole for now (approximation)
    rs_f = float(black_holes[0].r_s)
...
def main():
    ...
    BH1 = BlackHole([-3e10, 0.0, 0.0], 8.54e36)  # Left black hole
    BH2 = BlackHole([3e10, 0.0, 0.0], 8.54e36)   # Right black hole
    black_holes = [BH1, BH2]
    rays = []
    
    # Add rays coming from left at different heights
    start_x = -1e11
    for i in range(12):
        y_offset = -5e10 + i * 1e10
        rays.append(Ray([start_x, y_offset], [c, 0.0], black_holes))
    ...
    while not glfw.window_should_close(engine.window):
        ...
        for bh in black_holes:
            glPushMatrix()
            glTranslatef(bh.position[0], bh.position[1], 0.0)
            bh.draw()
            glPopMatrix()
        
        # Update and draw rays
        for ray in rays:
            ray.step(1.0)  # Same step size as original
